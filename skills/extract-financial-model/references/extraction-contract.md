...
| Explicit worksheet section, metric order, and display hierarchy | `TablePresentation` | Preserve a lean, non-canonical table grouping with ordered metric IDs, child-to-parent `metricParentIds`, and the source range. |
...
Emit one `tablePresentations` entry per defensible worksheet/view. A model may have multiple presentations when its canonical observations span multiple meaningful pages; do not split one calculation graph into fake models merely to preserve worksheets.
...
- Preserve defensible, section-relative display relationships in `metricParentIds`; omitted metric IDs are roots. Both child and parent must belong to the section, and the parent must appear earlier in reading order. The viewer derives depth from these edges instead of storing a render-time number.
...
The validator enforces presentation coverage and parent integrity automatically. Across a model's presentations, every observed metric must appear at least once; within each presentation it may appear only once. Every model, metric, section, presentation parent, semantic component parent, and source reference must resolve. If no presentation exists, add an open unresolved item with `category: presentation`; the validator emits a visible fallback warning. Missing both presentations and that unresolved item is an error.
...
The mapped XLSX extractor may auto-translate numeric and percentage literals, `+`, `-`, `*`, `/`, comparisons, restricted `IF`/numeric `IFERROR`/`MOD`, `SUM`/`AVERAGE` including multiple ranges, equal-length range-only `SUMPRODUCT`, and one-criterion `SUMIFS`/`AVERAGEIFS`. Semantic references must have an explicit metric/period mapping from a row/column grid mapping or explicit per-metric cells on any mapped worksheet. Formula-free numeric cells inside declared `periodHeaderRange`/`periodHeaderRanges` are the sole non-semantic references that may be constant-folded; arbitrary unmapped cells may not. A layout `INDIRECT(ADDRESS(ROW(), <index cell>))` pair resolves to the concrete same-row reference it names, but only when that index cell also sits inside a declared `periodHeaderRange`/`periodHeaderRanges` locator and holds a positive integer column number; any other `INDIRECT` remains an unsupported-function blocker. Omitted Excel arguments (`SUM(A1,,B1)`) are numeric zero. It follows Excel's numeric handling for mapped or materially blank references, including absent sparse OOXML cells on a known worksheet: direct arithmetic and `SUM` coerce them to zero, while `AVERAGE` excludes them. It must replay the restricted expression against source cached values and accept the translation only when the result matches the formula cell's cached result. Replay is a numerical safety check, not proof that a guessed metric or period meaning is correct; every semantic reference must already be explicit and defensible. Numeric `IFERROR` arithmetic with possible division by zero compiles to lazy conditional guards using the source fallback; a non-failing primary may be unwrapped only when it independently replays. An accepted source-derived translation takes precedence over a generic mapped expression because it preserves the actual period-specific lineage. Before leaving a formula opaque, expand the semantic map for every defensible referenced cell. The CLI writes every remaining translation blocker to `formula-translation-tasks.json`; the extraction agent must inspect and classify that queue. It may extend a private workbook map as part of extraction, but must not silently patch shared translator code unless tooling changes are explicitly in scope. Unsupported syntax or replay mismatch uses a reviewed mapped expression when one exists; otherwise preserve the formula as opaque and classify the remaining issue under Section 7. Never execute the source formula or agent-authored TypeScript/JavaScript.

Excel aggregate functions ignore text references; the translator may therefore omit or zero a referenced text value in `SUM` and exclude it from `AVERAGE`, but only when cached-value replay confirms the result. A numeric `IFERROR` whose primary branch deterministically reaches a nonnumeric source value may compile to its explicit source fallback when replay matches. These are source semantics, not guessed business values.
...
- `extraction_agent`: extend the map, translator, or deterministic transformation and rerun extraction; no analyst action is requested;
- `model_owner`: choose between materially different business/model interpretations that the source cannot disambiguate;
- `source_owner`: repair a broken reference, missing required value, or unusable upstream source.

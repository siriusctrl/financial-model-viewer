...
For every mapped formula, the extractor first attempts a deterministic translation of numeric and percentage literals, `+`, `-`, `*`, `/`, and restricted `SUM`/`AVERAGE` calls. Every referenced cell must resolve through either a grid mapping or an explicit cell mapping. Same-period inputs compile to `ref`; cross-period inputs compile to `period_ref(metricId, periodId)`. Mapped cross-sheet references are supported. Mapped blank references follow Excel numeric semantics: arithmetic and `SUM` use zero, while `AVERAGE` excludes blanks. The extractor replays the translated expression using workbook cached values and accepts it only when the result matches the formula cell's cached value. This exact source-derived lineage takes precedence over a generic metric-level `canonicalExpression`, which remains a reviewed fallback for formulas outside the deterministic subset.
...
Do not accept the first `unmapped_cells` or `unmapped_sheet` blocker as final. Inspect the named cells, extend the semantic map when their roles are defensible, and rerun until referenced-cell closure converges. If syntax remains unsupported or replay fails, preserve the exact formula and materialized value as `opaque` with a `needs_review` item whose next step names the engineering gap rather than asking the analyst to fix translator coverage. A formula with no cached value emits no fabricated observation and creates an `action_required` item. Comments attached to selected observation cells become evidence linked to those observations. Material comments on in-scope drivers should lead to mapping those drivers; comments outside the selected semantic graph remain complete in `workbook-inventory.json` and are counted in the report.

Human-facing attention items must print the mapped worksheet plus the narrowest cell or range. Cross-sheet blockers must also name every referenced worksheet and cell that needs mapping. Retaining that information only in a machine-readable locator is insufficient.

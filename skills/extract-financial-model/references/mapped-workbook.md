...
When the confirmed Leo convention applies, use `styleConvention: leo-yellow-green-blue@0.1`. It recognizes only encodings observed in the Leo workbook family:
...
A yellow literal is `leo_assumption`; a blue literal with no fill is `leo_reported_hardcode`; and a green-font or green-border formula is `leo_cross_sheet_reference` only when the formula itself contains an explicit worksheet reference. Green is positive evidence, not an exhaustive test: default/black formulas may still cross sheets. Formula presence always wins over yellow or blue styling and keeps the observation `derived`.

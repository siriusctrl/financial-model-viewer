...
If an otherwise supported formula references only unmapped period columns on an already mapped metric row, extraction fails with the formula worksheet, target cell, and missing source cells. This is an incomplete private map, not an analyst ambiguity: add the explicit periods and rerun. Partial-period scope is not allowed to silently downgrade selected formula lineage to `opaque`.

...
3. Build the semantic cell graph and objects.
   ...
   - Convert supported semantic hierarchy into `component_of` relationships. Require an explicit nullable `componentOfMetricId` decision for every mapped metric so omission cannot silently flatten the semantic graph. Do not use `component_of` merely to reproduce visual indentation: YoY, margin, bridge, and helper rows are often visually nested without being components.
   - When worksheet section labels and reading order are explicit, preserve them as ordered `tablePresentations` entries. Record each mapped metric's section-relative display parent as required nullable `presentationParentMetricId`; the extractor writes child-to-parent edges to `section.metricParentIds`, and the viewer computes depth at render time. Use one presentation per meaningful worksheet/view when a model spans multiple pages; keep cross-sheet observations in the same model so calculation lineage remains resolvable. Reference metric IDs and retain source ranges; never make a row or cell the metric identity.
   ...
   - For a stable but complex workbook, prefer `extract-mapped-workbook.py` plus a private explicit map over embedding company-specific branches in the reusable extractor. Use `financial-model-workbook-map@0.3` for new work: retain row/column mapping for genuine grids (optionally naming another worksheet on the metric), use per-metric `cells` for arbitrary layouts, and always declare both parent decisions. A non-null presentation parent must occur earlier in the same section; a non-null component parent must be another mapped metric. Missing, dangling, cyclic, or multiply-parented relationships are hard failures.

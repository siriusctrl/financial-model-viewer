...
3. Build the semantic cell graph and objects.
   ...
   - When a confirmed author convention applies, select its exact named `styleConvention` from `references/mapped-workbook.md`. Alice and Leo conventions are separate fixed evidence profiles, not configurable color rules. Treat matches as source/adjustability evidence, keep formulas `derived`, and never infer actual/estimate status from styling alone.

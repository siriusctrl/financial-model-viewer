...
3. Build the semantic cell graph and objects.
   ...
   - Convert supported semantic hierarchy into `component_of` relationships. Do not use it merely to reproduce visual indentation: YoY, margin, bridge, and helper rows are often visually nested without being components.
   - Preserve explicit worksheet sections, reading order, and display parents in `tablePresentations`. Keep view-specific parents independent from semantic `component_of` relationships; the viewer derives indentation from parent edges at render time.
   ...
   - For a stable but complex workbook, prefer `extract-mapped-workbook.py` plus a private explicit map over company-specific extractor branches. Use the current map syntax and hierarchy review gate documented only in `references/mapped-workbook.md`.
   - When a confirmed formatting convention applies, select its exact named `styleConvention` from `references/mapped-workbook.md`. Public conventions must use neutral names and fictional examples; never add a convention named after a real author. Alice remains a legacy exception and is not a naming precedent. Treat matches as source/adjustability evidence, keep formulas `derived`, and never infer actual/estimate status from styling alone.
   - Before delivery, use a privacy-safe `sourceArtifact.title`. Extraction output must omit workbook/comment authors, creator/last-modifier metadata, local source paths, and person-bearing source filenames. Preserve source identity with the content hash and workbook cell locators instead.

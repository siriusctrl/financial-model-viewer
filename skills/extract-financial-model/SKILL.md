...
4. Translate calculations.
   ...
   - Translate supported formulas to `model-expression@0.1`. For mapped XLSX files, let the deterministic translator convert numeric/percentage literals, basic arithmetic and comparisons, restricted `IF`/`IFERROR`/`MOD`, `SUM`/`AVERAGE`, and one-criterion `SUMIFS`/`AVERAGEIFS` only when semantic inputs have canonical metric/period metadata and cached-value replay matches; cross-period inputs must use exact `period_ref` references. Formula-free numeric cells inside declared `periodHeaderRange`/`periodHeaderRanges` may be constant-folded, but arbitrary unmapped cells may not. Mapped or materially blank references follow Excel numeric coercion (`0` in arithmetic and `SUM`, excluded from `AVERAGE`).

...
When open attention exists, select the status in the header to open **What needs attention**. Each current extraction item states what the extractor did, why it matters, and the exact next step. `action_required` items need a source or extraction change and cannot be cleared in the viewer. A `needs_review` item can be confirmed only when all three explanations are present; older portable datasets with incomplete guidance remain inspectable but cannot be confirmed. Selecting an item switches models, reveals its source worksheet/cell, and highlights the affected table context. The cell inspector is a dismissible slide-over and stays hidden until a cell or issue is selected. On layouts wide enough to keep the table visible, it docks to whichever side overlaps the selected cell least; narrow screens retain the focused near-full-screen card.

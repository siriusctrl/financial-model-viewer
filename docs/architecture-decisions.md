...
The extraction-package checker also requires every open unresolved item to be named in the report with its exact attention level and to carry three user-facing fields: `currentTreatment`, `impact`, and `nextAction`. A valid database with an incomplete or unintelligible handoff is not a valid extraction package. These fields are optional at the runtime-schema boundary so older portable datasets remain inspectable, but new extraction packages fail the strict checker when the guidance is absent.

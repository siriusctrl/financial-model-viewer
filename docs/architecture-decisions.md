...
The extraction-package checker also requires every open unresolved item to be named in the report with its exact attention level and every attention item emitted by the package to carry three user-facing fields: `currentTreatment`, `impact`, and `nextAction`. Each action additionally assigns `extraction_agent`, `model_owner`, or `source_owner`; this prevents an engineering limitation from appearing as an analyst question. A valid database with an incomplete or unintelligible handoff is not a valid extraction package. These fields are optional at the runtime-schema boundary so older portable datasets remain inspectable, but incomplete legacy review items cannot be confirmed and new extraction packages fail the strict checker when guidance or ownership is absent.
